...
class Probably<T> implements Actionable<T>, Immutatorable<T>, Applicable<T> {
   ...
   * @param value the value for class attribute T
   */
   ...
   * 
   * @param <T> the type of Probably class
  ...
  public Probably<T> check(Immutator<Boolean, ? super T> ime) {
    
    if (this.value != null) {

      if (ime.invoke(this.value)) {
  ...
  public <R> Probably<R> apply(Probably<? extends Immutator<? extends R, ? super T>> immutator) {
    ...
    if (this.value != null && immutator.value != null) {
      ...
      return Probably.just(immutator.value.invoke(this.value));
